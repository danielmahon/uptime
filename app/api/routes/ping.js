...
module.exports = function(app) {
	...
	app.get('/pings', function(req, res, next) {
		...
		Ping.find(query).sort({
			timestamp: -1
		}).limit(50).skip((req.param('page', 1) - 1) * 50).exec(function(err, pings) {
			if (err)
				return next(err);
			res.json(pings);
		});
	...
	app.get('/pings/check/:id/:page?', function(req, res, next) {
		Check.count({
			_id: req.params.id
		}, function(err, nb_checks) {
			if (err)
				return app.next(err);
			if (!nb_checks)
				return res.json(404, {
					error: 'failed to load check ' + req.params.id
				});
			Ping.find({
				check: req.params.id
			}).sort({
				timestamp: -1
			}).limit(50).skip((req.params.page - 1) * 50).exec(function(err, pings) {
				if (err)
					return next(err);
				res.json(pings);
			});
	...
	app.post('/pings', function(req, res) {
		...
		function getCheck(checkId) {
			Check.findById(checkId, function(err1, check) {
				if (err1) {
					return res.send(err1.message, 500);
				}
				if (!check) {
					return res.send('Error: No existing check with id ' + checkId, 403);
				}
				if (!check.needsPoll) {
		...
		// create time delta for reverse pings
		if (req.body.type && req.body.type === 'reverse') {
			req.body.time = Date.now() - req.body.timestamp;
			Check.findOne({
				url: 'reverse://' + req.body.path
			}, function(err, check) {
				if (err || !check) {
					getCheck();
					return;
